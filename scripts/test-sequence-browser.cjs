...
(async()=>{
  ...
  const go=async()=>{
    ...
    await page.locator('[data-level="s1"]').waitFor();
  };
  await go();
  assert.equal(await page.locator('#mode-sequence').getAttribute('aria-current'),'page');
  assert.equal(await page.locator('[data-level="s1"]').isDisabled(),true);
  ...
  assert.equal(await page.locator('[data-level="s1"]').isDisabled(),false);
  ...
  await page.reload(); await page.locator('[data-level="s5"]').click();
