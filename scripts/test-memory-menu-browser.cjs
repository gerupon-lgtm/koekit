  ...
  try {
    ...
    for (const width of [320, 390, 768]) {
      ...
      for (const sequence of [false, true]) {
        await page.goto('http://127.0.0.1:8000/kioku/' + (sequence ? '?mode=sequence' : ''));
        ...
        assert.equal(await page.locator('[data-level]').evaluateAll(es => es.every(e => e.clientWidth >= e.scrollWidth && e.getBoundingClientRect().height >= 64)), true);
        assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
        const initial = await Promise.all(['.app-title', '.mode-picker', '.memory-speed-picker', '#highest-title'].map(top));
        positions.push(initial);
        await page.screenshot({ path: `.local-tools/menu-${sequence ? 'sequence' : 'place'}-${width}.png`, fullPage: true });
        const key = 'koekit.progress.v1.' + (sequence ? 'kioku-sequence' : 'kioku-place');
        ...
        const switched = await Promise.all(['.app-title', '.mode-picker', '.memory-speed-picker', '#highest-title'].map(top));
    ...
    console.log('PASS memory menu: labels, locks, keyboard switching, clear marks and aligned layout at 320/390/768px');
