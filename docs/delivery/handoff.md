...
2026-09-27更新。要件v0.38／基本設計v0.3。**本体・アセットは2026-09-26にGitHub Pages公開済み。HTTPSでのタッチ操作と配信ファイル一致を確認。** [実装記録](implementation-log.md)を先に読み、実機受入の残件と検証コマンドを確認する。ミニメロディの要件定義は別作業として進め、新作同士の仕様を混ぜない。

音声不具合修正の根拠・参照した既存ソース・実モデル検証は[音声確認記録](speech-review.md)を参照。認識結果の模擬注入だけを実音声の検証とは扱わない。

全面クリアの一枚絵と短い音楽を追加。難易度別の全4レベル達成後、結果の「つぎ」で表示する。承認済み原本とWebPは `assets/delivery/`。検証と保存項目は実装記録の2026-09-27節を参照。
...
2. [デリバリズム要件書](../requirements-robot.md) v0.38。
...
新規delivery・専用素材・トップ導線・SW事前保存・版検証を追加済み。既存4作品・共通srcの74ファイルは開始前後ハッシュ一致。GitHub Pages公開済み。更新時は既存版管理とキャッシュ手順に従い、公開状況と実機検証状況を別々に報告する。
