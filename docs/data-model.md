...
ジントリズム（未実装）のモード・難易度別の最新1件保存、色・先攻・規定局数の成績・延長状態、連勝・正式終了は [基本設計書7節](jintorhythm-design.md#7-データと自動保存) を参照。盤面サイズ・局数ごとには保存枠を増やさない。以下の既存作品の旧「保存しない」記述をジントリズムへ適用しない。
...
## デリバリズム（ローカル実装済み、2026-09-26）

[データ設計](delivery/design.md)7節を正典とする。Stage/Runtime/Sequence/Session/Library/Progressを分離し、終了削除を途中状態へ限定する。保存枠・復帰操作は同書D-Q02で確定（最新1件・確定stepで停止復帰）。既存作品の保存キーは流用しない。
