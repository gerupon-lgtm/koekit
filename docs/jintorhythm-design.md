...
版：0.3／作成・更新：2026-09-20／状態：ローカル実装・自動検証済み。実機発話・PWA受入と公開は未実施。
...
JT-T01〜05のローカル実装と自動検証を実施。JT-T06はトップ・SW・版表示・既存作品回帰まで実施し、実機受入・公開確認が残る。検証範囲と再実行手順は [実装記録](jintori-implementation-log.md) を参照。回数やAIの強さの実機調整と、基本ロジックの正しさの検証を分ける。
