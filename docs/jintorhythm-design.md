...
版：0.4／更新：2026-09-21／状態：実装・自動検証・HTTPS公開済み。Android/iOS実機発話・PWA受入は未実施。

### 2026-09-21 追加合意：終盤の特殊石とプレビュー

- 毎局補充のみ、現手番から交互に空きを埋める場合の残り回数 `ceil(空きマス数 / 2)` が2以下なら、在庫のある強化→最強の順で自動選択・使用必須。両プレイヤー・CPUに適用する。4×4は従来どおり特殊不可。
- 残り回数は推定値。今後の置き方やパスを予知するものではない。パスでは消費せず、双方合法手なしで早期終局する場合も残った在庫を架空消費しない。
- 人の手番は自動選択後も座標→プレビュー→必要なら方向番号→オッケーを維持。取消・位置変更・再開だけでは消費しない。自動選択中に通常石や逆順へ切り替えて回避できない。
- 持ち越しは従来どおり、人のプレイヤーが発動を選ぶまで自動使用しない。CPUの既存の判断方式は維持。
- 置くマスは実線＋「＋」、反転するマスは破線＋「↻」。座標と反転数を表示し、確定前であることを区別する。最強の同数方向選択は維持。
- 保存形式は変更せず、再開時にも現在の盤面と在庫から自動選択を再計算する。以下の従来の「1手限りの手動選択」は毎局補充の終盤だけこの追加合意で上書きする。
...
2026-09-20追加指定：画面文言はひらがな中心。メニュー・結果・説明・エラー・読み上げ用ラベルも揃える。以下の設計用名称（規定局数・毎局補充など）は画面上では「かいすうをきめる」「まいかいほじゅう」などと表す。トーストは20〜24px太字、3200ms表示とする。
...
2026-09-20追加指定：CPUが即座に返答しても、考える表示から最低600ms待って着手する。待機中の終了・説明・非表示は可能で、古い待機処理をキャンセルする。サイコロ回転中は短い控えめな音を数字更新に合わせて鳴らし、ストップ発話を受け付ける。ストップ後は認識を停止し、数字と音の間隔を広げて音程も下げる。着手・結果などの効果音は従来どおり認識停止区間で鳴らす。
...
## 2026-09-21 追加合意：枚数ヒントの切替

- 盤面下部の反転枚数テキストをプレイ中にON／OFFできる。画面上部の「ヒント」ボタンで操作する。
- 初期値はやさしいのみON。ふつう・むずかしい・ふたりはOFF。通常石・強化・最強で同じスイッチを使う（発案者合意）。
- OFFでも盤面の置くマス／反転対象プレビュー、石の種類、座標、無効な位置の案内、同数方向の番号・矢印と選択は残す。方向候補の枚数テキストはスイッチに連動する。
- 表示設定はゲーム進行・CPU強さ・特殊石消費に影響しない。切替時も現在の選択、音声区間、CPU待機を維持する。
- 実装上の保持範囲：現在のプレイ中（次局・延長を含む）。新規開始／再開時は難易度の初期値とし、途中保存スキーマは変えない。
- 検証：`scripts/test-jintori-hints-browser.cjs`。4設定の初期値、選択後の即時切替とプレビュー維持、特殊石、同数方向、CPU待機、小画面の確定ボタンを確認。
