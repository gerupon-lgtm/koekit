...
計画・仕様：`docs/jintorhythm-design.md` v0.3（JT-T01〜06）。開始：2026-09-20。実装・自動検証・HTTPS公開済み。Android/iOS実機発話・PWA受入は未実施。
...
`jintori/app.js` が画面・音声区間・サイコロ・CPU Worker・保存を接続する。イロドリズムの12色と座標語彙、共通音声ファクトリ・マイク表示・消灯防止・ルーレットを使用。効果音は追加指定により専用の `sound.js` で合成。既存作品のコントローラは変更していない。メニューは切替ボタンにまとめ、クリーム・緑・紫の配色と共通ズム部品のロゴを追加した。
...
- 公開済み：`https://koekit.sikumilab.com/jintori/`。GitHub Pagesはmainのルートを配信。初回公開コミット `2dd1cd6` のbuild成功、HTML・音源・ビュー・SWのHTTP 200と公開版の基本操作を確認。発案者の承認範囲に含まれるファイルだけをコミットし、先行する無関係な文書・音声層の作業を含めていない。
- JT-T06の公開工程は実施済み。実機受入と全体目標は未完了。公開URLのPCブラウザ検証をAndroid/iOSの実発話確認の代わりにしない。
