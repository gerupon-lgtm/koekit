...
| 検証 | 結果・範囲 |
|---|---|
| 新規Nodeテスト7ファイル | 56ケース成功。ルール19、CPU11、発話解析5、対局進行6、保存8、音声区間3、効果音4 |
...
### 2026-09-20 実機フィードバックの追加調整

- サイコロの数字更新ごとに短い回転音を鳴らす。回転中は「ストップ」を受け付けるため控えめな音量、停止指示後は認識を止めて少し大きくする。数字と音が同じ間隔で減速し、音程も下がる。共通ルーレットを変更せず、ジントリズムの設定だけを調整。
- CPUは考える表示から最低600ms待って着手する。計算と並行した待機であり、600msを超える計算にさらに600ms加算しない。終了・非表示・説明で待機を取り消す。
- トーストは20〜24px・太字・行間1.5、表示3200ms。自動パスは陣営名と理由を改行して表示する。
- メニュー・盤面・結果・説明・通知・読み上げ用ラベルをひらがな中心へ変更。「はじめから」「つづきから」「まいかいほじゅう」「もちこし」「おわる」など。保存キー・ゲームルール・既存の発話同義語は変更しない。
- `test-jintori-feedback.cjs` で、即応するCPU Workerでも600ms待つこと、回転音中のストップ発話、停止後の音の間隔増加と認識停止、320px幅の大きいトーストを検証。既存の画面・効果音・保存異常系テストも成功。
