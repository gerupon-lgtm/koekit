...
現行ロゴ画像と色味を維持。上寄せの共通配置と5ボタン分の領域により、場所／順番、通常／スピードでロゴ・記録の表示位置を揃える。小画面は縦スクロール。重複するタイトルの開始ボタンは除去し、内容ボタンから開始する。

検証：`test-memory-menu-browser.cjs` で320/390/768pxの配置一致、文字の収まり、ロック／解放、キーボード切り替え、クリア表示を確認。既存のprogression/sequenceブラウザテストも通過。
