...
設計と確定仕様は [基本設計v0.3](jintorhythm-design.md)、実装範囲・再実行手順・結果は [実装記録](jintori-implementation-log.md)。新規Node55ケース、Chrome/WebKit基本操作、ChromeのWorker/競合/容量不足/オフライン、既存3作品回帰が成功。実発話・Android/iOS PWA受入と公開は未実施。実装前から存在する `src/speech/vosk.js` の未コミット変更は触っていない。

現在の公開版がジントリズムに対応済みという意味ではない。公開前に先行変更を含む差分範囲を整理する。
