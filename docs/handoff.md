...
設計と確定仕様は [基本設計v0.3](jintorhythm-design.md)、実装範囲・再実行手順・結果は [実装記録](jintori-implementation-log.md)。新規Node55ケース、Chrome/WebKit基本操作、ChromeのWorker/競合/容量不足/オフライン、既存3作品回帰が成功。実発話・Android/iOS PWA受入は未実施。公開状況は次段落を参照。実装前から存在する `src/speech/vosk.js` の未コミット変更は触っていない。
